...
def abc_algorithm(tasks, num_bees=10, max_iter=100):
    ...
    for _ in range(num_bees):
        schedule = [{'task': task['name'], 'duration': random.randint(1, 3), 'time_slot': random.randint(1, 5)} for task in tasks]
    ...
    for iteration in range(max_iter):
        for bee in bees:
            ...
            new_schedule.append({'task': random_task['task'], 'duration': random.randint(1, 3), 'time_slot': random.randint(1, 5)})
